...
class Solution{
    ...
    void rightView(ArrayList<Integer> arr, Node node, int level){
        // if node is null simply return
        if(node == null){
            return;
        }
        // if it's the last node of it's level then it is the right view
        if(maxLevel<level){
        ...
        rightView(arr, node.right, level+1);
        rightView(arr, node.left, level+1);
    }
